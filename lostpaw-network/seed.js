...
async function seedData() {
  try {
    ...
    const reports = await Report.createEach([
      ...
      // Lost dogs
      { id: new ObjectId().toString(), status: 'lost', category: 'dogs', description: 'Lost black Labrador.', location: JSON.stringify({ lat: 40.712776, lng: -74.005974 }), latitude: 40.712776, longitude: -74.005974, contactInfo: allUsers.find(user => user.email === 'jane@example.com').contactInfo, user: allUsers.find(user => user.email === 'jane@example.com').id, pet: pets[3].id, photoUrls: ['https://placedog.net/400/300'] },
      { id: new ObjectId().toString(), status: 'lost', category: 'dogs', description: 'Lost German Shepherd.', location: JSON.stringify({ lat: 40.706192, lng: -74.008584 }), latitude: 40.706192, longitude: -74.008584, contactInfo: allUsers.find(user => user.email === 'jane@example.com').contactInfo, user: allUsers.find(user => user.email === 'jane@example.com').id, pet: pets[4].id, photoUrls: ['https://placedog.net/600/300'] },
      { id: new ObjectId().toString(), status: 'lost', category: 'dogs', description: 'Lost Dalmatian.', location: JSON.stringify({ lat: 40.730610, lng: -73.935242 }), latitude: 40.730610, longitude: -73.935242, contactInfo: allUsers.find(user => user.email === 'jane@example.com').contactInfo, user: allUsers.find(user => user.email === 'jane@example.com').id, pet: pets[5].id, photoUrls: ['https://placedog.net/500/300'] },
      
      // Found dogs
      { id: new ObjectId().toString(), status: 'found', category: 'dogs', description: 'Found black Labrador.', location: JSON.stringify({ lat: 40.712776, lng: -74.005974 }), latitude: 40.712776, longitude: -74.005974, contactInfo: allUsers.find(user => user.email === 'jane@example.com').contactInfo, user: allUsers.find(user => user.email === 'jane@example.com').id, pet: pets[3].id, photoUrls: ['https://placedog.net/400/300'] },
      { id: new ObjectId().toString(), status: 'found', category: 'dogs', description: 'Found German Shepherd.', location: JSON.stringify({ lat: 40.706192, lng: -74.008584 }), latitude: 40.706192, longitude: -74.008584, contactInfo: allUsers.find(user => user.email === 'jane@example.com').contactInfo, user: allUsers.find(user => user.email === 'jane@example.com').id, pet: pets[4].id, photoUrls: ['https://placedog.net/600/300'] },
...
sails.lift({}, async function(err) {
  ...
  try {
    ...
    sails.lower(() => {
      ...
      process.exit(); // Ensure the process exits after lowering the Sails app
    });
  }
});
